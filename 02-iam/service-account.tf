  ...
  _service_accounts = [for i in var.service_accounts : merge(i,
    {
      ...
      iam_folder_roles     = { for k, v in i.iam_folder_roles : replace(replace(k, "[PREFIX]", "${local.fldr_prefix}-"), "[ROOT]", local.parent_root) => v }
...
module "service-account" {
  ...
  project_id             = each.value.parent_complete_name == "BOOTSTRAP" ? local.bootstrap_id : try(local.projects[each.value.parent_complete_name].id, each.value.parent_complete_name)
  ...
  iam_storage_roles      = { for k, v in each.value.iam_storage_roles : try(local.folders[k].id, k) => v }
  ...
  iam_folder_roles       = { for k, v in each.value.iam_folder_roles : try(local.folders[k], k) => v }
